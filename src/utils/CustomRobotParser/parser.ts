...
const USER_AGENT_2 = 'useragent';
const ALLOW = 'allow';
const DISALLOW = 'disallow';
const SITEMAP = 'sitemap';
const CRAWL_DELAY = 'crawl-delay';
const CRAWL_DELAY_2 = 'crawldelay';
const HOST = 'host';
...
const parser = (rawString) => {
    ...
    const robotsObj: { sitemaps: any[]; host?: any } = {
        sitemaps: new Array<any>(),
    };
    let agent = '';

    lines.forEach((line) => {
        const record = parseRecord(line);
        switch (record.field.toLowerCase()) {
            case USER_AGENT:
            case USER_AGENT_2:
            ...
            case ALLOW:
                if (agent.length > 0 && record.value.length > 0) {
                    robotsObj[agent].allow.push(
                        groupMemberRecord(record.value)
                    );
                }
                break;
            case DISALLOW:
                if (agent.length > 0 && record.value.length > 0) {
                    robotsObj[agent].disallow.push(
                        groupMemberRecord(record.value)
                    );
                }
                break;
            ...
            case CRAWL_DELAY_2:
                if (agent.length > 0) {
                    robotsObj[agent].crawlDelay = Number.parseInt(
                        record.value,
                        10
                    );
                }
